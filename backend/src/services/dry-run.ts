...
export async function runDryRunMorning(userId: string): Promise<DryRunBundle> {
  ...
  for (const candidate of candidates) {
    ...
    const entryPrice = numeric(candidate.limitPrice) || numeric(candidate.triggerPrice) || latestPrice || 0;
    ...
    await db.insert(orders).values({
      ...
      gttCandidateId: candidate.tradeCandidateId ? null : candidate.id,
      tradeCandidateId: candidate.tradeCandidateId ?? null,
      placedAt: new Date(),
    });
  }
...
export async function completeDryRunDay(userId: string, tradeDate = indianTradeDate()): Promise<DryRunBundle | null> {
  ...
  for (const trade of bundle.trades) {
    ...
    const pnl = markToMarket(trade.transactionType ?? 'BUY', Number(trade.quantity), Number(trade.averagePrice), exitPrice);
    totalPnl += pnl;
    ...
    await db.update(orders).set({ currentPrice: String(exitPrice), exitPrice: String(exitPrice), pnl: String(pnl), status: 'dry_run_closed', updatedAt: new Date() }).where(eq(orders.id, trade.id));
  }

  const summary = `Dry-run EOD RCA: ${signals.length} simulated position(s), total hypothetical PnL ${money(totalPnl)}. Findings are stored as RCA and supplied to future morning research.`;
  const [session] = await db.update(dailyResearchSessions).set({ dryRunStatus: 'completed', dryRunTotalPnl: String(totalPnl), dryRunSummary: summary, dryRunCompletedAt: new Date(), updatedAt: new Date() }).where(eq(dailyResearchSessions.id, bundle.session.id)).returning();
  const [report] = await db.insert(rcaReports).values({
    userId,
    researchSessionId: session.id,
    tradeDate,
    reportType: 'dry_run_eod',
    dailySummary: summary,
    strategyReview: totalPnl >= 0 ? 'Dry-run basket was net positive or flat; review winners for repeatable evidence patterns.' : 'Dry-run basket was net negative; review losers for thesis, trigger, and market-regime mismatch.',
    agentReasoningReview: 'Generated from dry-run outcomes. This RCA should influence future research prompts without changing execution safety controls.',
    riskReview: 'No broker orders were placed. Risk was limited to simulated exposure only.',
    ...
    raw: { signals, totalPnl },
  }).returning();

  for (const signal of signals) {
    const trade = bundle.trades.find((item) => item.tradingsymbol === signal.symbol);
  ...
  await audit('dry_run.eod.rca.complete', { userId, entityType: 'rca_report', entityId: report.id, metadata: { tradeDate, totalPnl, trades: signals.length } });
...
function candidatesForDryRun(research: Awaited<ReturnType<typeof runMorningResearch>>): Candidate[] {
  ...
  return research.tradeCandidates.map((item) => ({ id: item.id, tradeCandidateId: item.id, exchange: item.exchange, tradingsymbol: item.tradingsymbol, transactionType: item.side, quantity: 1, rationale: item.thesis, raw: item.raw }));
}
