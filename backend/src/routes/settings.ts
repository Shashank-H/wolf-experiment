...
export const settingsRoutes = new Elysia({ prefix: '/settings' })
  ...
  .put('/providers', async ({ body, cookie, set }) => {
    ...
    const providerConfig = {
      llmBaseUrl: typeof input.llmBaseUrl === 'string' ? input.llmBaseUrl : undefined,
    ...
    await audit('settings.providers.update', { userId: user.id, metadata: { providers: ['kite', 'exa', 'finnhub', 'llm'] } });
    return { ok: true };
  })
  .put('/yolo-mode', async ({ body, cookie, set }) => {
    ...
    if (enabled && input.confirmation !== 'I understand YOLO mode still requires risk checks') {
      set.status = 400;
      return { error: 'YOLO mode requires explicit confirmation copy' };
    }
