...
| Research source setup | Morning research no longer requires hardcoded seed symbols or an Exa/Finnhub key as a setup preflight; it still requires an LLM API key. Missing source providers should result in empty/limited catalyst discovery with explicit warnings, not invented symbols. | The user can see that no fresh external catalyst evidence was available. Broker/portfolio context is not converted into fake market discovery. |
| Research provider runtime failures | Configured source-provider failures are captured as discovery warnings. The run may continue with whatever real evidence remains, but candidate generation must stay grounded in provider/classifier symbols, broker context, or sources. | The failed reason is recorded in warnings/error metadata. Unsupported symbols and ungrounded GTT drafts are rejected by validation. |
...
| Small-model catalyst classification | The small model classifies pre-market sources into explicit NSE symbols, catalyst type/direction/strength, and confidence. If it fails, the service falls back only to provider-supplied symbols with `other_news` / `unknown`; it does not infer symbols from arbitrary title/summary text. | Candidate discovery remains conservative and auditable; no regex-derived or invented ticker is promoted because classification failed. |
...
- No production hardcoded default research symbols.
- No primary pre-market discovery from after-the-fact NSE top gainers/losers unless explicit reactive confirmation is enabled.
