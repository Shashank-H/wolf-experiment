...
const defaultOrder = JSON.stringify({ action: 'flag_candidate', message: 'Review this symbol in Wolf when the rule matches.' }, null, 2);
      ...
      <Card title="New trigger" marker="[+]">
        <form className="stack" onSubmit={(event) => { event.preventDefault(); create.mutate(); }}>
          ...
          <div className="field-grid">
            ...
            <Field label="Internal action JSON" info="App-only payload recorded when the trigger matches. Triggers never place broker orders or call broker execution APIs."><textarea value={orderDraft} onChange={(event) => setOrderDraft(event.target.value)} rows={10} /></Field>
          </div>
          ...
          <button disabled={create.isPending}>{create.isPending ? 'Creating…' : 'Create internal trigger'}</button>
        </form>
